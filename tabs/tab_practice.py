...
""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        # 发牌按钮
        if st.button(t("deal_btn"), use_container_width=True, type="primary", disabled=st.session_state.end_shoe):
        ...
        # 新牌靴按钮
        if st.button(t("new_shoe"), use_container_width=True):
            reset_logic()
            st.rerun()
    ...
    with col_right:
        ai_snap = {
            "status": "SCANNING...",
            "fingerprint": current_rk, # 🚨 修改：移除 "OUT-" 前缀和截断，显示原始 18 位 Key
            "similarity": f"{min(99.9, (1 - len(st.session_state.shoe)/416)*100 + 20):.1f}%"
        }
        render_snapshot_ai(ai_snap, lang=st.session_state.lang)
